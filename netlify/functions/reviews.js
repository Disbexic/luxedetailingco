...
export default async (request) => {
  try {
    ...
    if (String(data._smoke || "") === "1") {
      ...
      const afterVerify = (await store.get(KEY, { type: "json" })) || [];
      await store.setJSON(KEY, afterVerify.filter((item) => item.id !== entry.id));
    ...
    if (!stored) {
      return json({ error: "Review could not be verified after saving." }, 500);
    }
